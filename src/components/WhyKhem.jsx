      ...
      <div className="container-khem">
        ...
        <div className="mt-10 grid gap-3 lg:grid-cols-[repeat(3,minmax(0,1fr))_2.8fr]">
          ...
          <motion.article initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: .24 }} className="texture-dark grid min-h-[310px] overflow-hidden rounded-xl text-white shadow-xl sm:grid-cols-[1.2fr_1fr]">
            <div className="relative min-h-[260px] overflow-hidden"><Image src="/youtube-award.webp" alt="Khem Tattoo YouTube Silver Creator Award" fill sizes="(max-width: 640px) 100vw, 35vw" className="-rotate-90 scale-[1.45] object-cover object-center" /><div className="absolute inset-0 bg-gradient-to-r from-transparent to-black/45" /></div>
            <div className="flex flex-col justify-center p-7"><span className="text-xs font-bold uppercase tracking-wider text-[#ba9255]">100K+ Strong</span><h3 className="mt-2 font-display text-4xl">Thank you!</h3><div className="my-4 h-0.5 w-10 bg-[#ba9255]"/><p className="text-sm leading-7 text-white/75">This milestone is yours as much as ours. Thank you for trusting and supporting our journey.</p></div>
          </motion.article>
        </div>
      </div>
    </section>
